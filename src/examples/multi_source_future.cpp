...
cocls::task<void, cocls::resumption_policy::parallel> calc_val(int i, int delay, cocls::promise<int> p) {
...
cocls::task<int> collector(int i, int j, int d1, int d2) {
    cocls::multi_source_future<int> fut;
    calc_val(i,d1, fut.get_promise());
    calc_val(j,d2, fut.get_promise());
